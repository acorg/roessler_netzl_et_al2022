...
for(n in n_samples) {
    ...
    for(i in 1:10){
        ...
        sub_map <- optimizeMap(sub_map, 2, 500, options = list(ignore_disconnected = TRUE))
        sub_map <- realignMap(sub_map, neut)
        save.acmap(sub_map, paste0("revision/subset_serum_maps/n_serum_",n, "_run_", i, ".ace"))
    }
...
png(paste0("revision/subset_serum_maps/",n,"_sera.png"), width = 8, height = 3, units = 'in', res=300, pointsize = 18)
layout(matrix(c(1:10), ncol = 5, byrow = T))
par(oma=c(0, 0, 0, 0), mar=c(0.1, 0, 1, 0))

for(i in 1:10){

    p <- read.acmap(paste0("revision/subset_serum_maps/n_serum_",n,"_run_", i, ".ace"))
